module.exports =  {
    parserPreset: {
        parserOpts: {
            ...
            headerCorrespondence: ['type','ticketKey', 'ticketNumber', 'description'],
        }
    },
    plugins : [{
        rules: {
            'header-format': (parsed, _when) => {
                ...
                if (!headerPattern.test(parsed.header)) {
                    throw new Error('❌ Invalid commit message format!(check for any extra whitespace character.) Expected format: "type: ticketKey-ticketNumber description');
                }
                return [true];
            },
            ...
            'ticket-key-format': (parsed, _when, expectedValues) => {
                ...
                if(ticketKey.length > expectedValues) {
                    return [false, `❌ ticketKey length must be atmost ${expectedValues} characters. The current length is ${ticketKey.length} characters`];
                }
            ...
            'description-min-length': (parsed, _when, expectedValues) => {
                ...
                if (!description || description?.length < expectedValues) {
                    return [false, `Description must be at least ${expectedValues} characters. The current length is ${description.length || 0} characters`];
                }
                return [true];
            },
            'description-max-length': (parsed, _when, expectedValues) => {
                const { description } = parsed;
                if (!description || description?.length > expectedValues) {
                    return [false, `Description length must be at most ${expectedValues} characters. The current length is ${description.length || 0} characters`];
                }
                return [true];
            },
        }
